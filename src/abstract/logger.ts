...
if (Environment.APP_LOGGING_LEVEL !== undefined) {
	logger.createConsoleTransport(Environment.APP_LOGGING_LEVEL);
}
else {
	logger.createConsoleTransport(process.env.NODE_ENV === 'production' ? LogLevel.Information : LogLevel.Debug);
}
	...
	log(message: any, ...optionalParams: any[]) {
		if (typeof optionalParams[0] === 'string') {
			if (DebugNames.has(optionalParams[0])) {
				...
				return this.verbose(message, ...optionalParams);
			}
		}
